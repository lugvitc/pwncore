...
metadata = {"name": "auth", "description": "Authentication using a JWT using a single access token."}
...
async def signup_team(team: SignupBody, response: Response):
    try:
        ...
        await Team.create(
            name=team.name,
            secret_hash=bcrypt.hash(team.password)
        )
    except Exception:
...
@router.post('/login')
async def team_login(team_data : LoginBody):
